...
import { AudioVisualiser }  from './AudioVisualiser.jsx'
...
function App() {
  ...
  useEffect(() => {
    const update = async () => {
      ...
      bufferSrc.buffer = await fetchAudioBuffer("/media/singing.mp3");
      bufferSrc.loop = true;
      // connect
  ...
  return (
    <>
      ...
      <p></p>
      { audioNodes?.bufferSrc && <AudioVisualiser timeDataArray={Array.from(audioNodes.bufferSrc.buffer.getChannelData(0))} sampleRate={audioCtx.sampleRate}/> }
    </>
  )
