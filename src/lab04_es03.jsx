...
function getNextOctave(refFreq) {
    return 2 * refFreq;
}
...
function App() {
    ...
    function start() {
        ...
        if (audioNodes?.osc) {
            ...
            for (let i = 0; i < audioNodes.gains.length; i++){
        ...
        for (let i = 0; i < majorScale.length; i++) {
            ...
            for (let c = 0; c < 3; c++) {
                let freqIndex = i + 2 * c;
                let noteF = (freqIndex < majorScale.length) ?
                    majorScale[freqIndex] :
                    getNextOctave(majorScale[freqIndex % (majorScale.length - 1)])

                osc.push(new OscillatorNode(audioCtx, {frequency: noteF}));

                osc[osc.length - 1].connect(gains[i]);
            }
