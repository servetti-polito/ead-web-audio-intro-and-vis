...
function App() {
    ...
    const defaultModulatorFrequency = 20;
    ...
    return (
        <>
            ...
            <Slider label={"Modulator Frequency"}
                    name="modFrequency" min={0} max={20} step={1}
                    value={modFrequency}
                    handleChange={(ev) => setModFrequency(ev.target.value)}
            />
            <p></p>
            <Slider label={"Modulator Gain"}
                    name="ampValue" min={0.1} max={100} step={0.05}
                    value={modAmp}
